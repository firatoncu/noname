...
async def tpsl_checker(symbol, current_position, pricePrecisions, client, logger):
    ...
    if get_order_status(symbol) == "LONG" and get_limit_order(symbol) != "False":
        ...
        if order_status['status'] == 'FILLED':
            ...
            if get_notif_status() == True:
                profit = round(abs(current_position) * (limit_tp_price - entry_price), 2)
    ...
    if current_position > 0:
        ...
        limit_tp_price = round(entry_price * 1.003, pricePrecisions[symbol]) # %1.5
        tp_price = round(entry_price * 1.004, pricePrecisions[symbol]) # %2
        sl_price = round(entry_price * 0.99, pricePrecisions[symbol]) # %5
        hard_sl_price = round(entry_price * 0.983, pricePrecisions[symbol]) # 8.5
        # First, try to place a limit order at limit_tp_price
        try:
            if get_order_status(symbol) == "False":
                limit_order = await client.futures_create_order(
                    symbol=symbol, 
                    side=SIDE_SELL, 
                    type="TAKE_PROFIT_MARKET", 
                    stopPrice=limit_tp_price,
                    closePosition=True
                )
                set_order_status("LONG", symbol)
                set_limit_order(limit_order, symbol)
                
                # Check if the limit order is filled
    ...
    if current_position < 0:
        entry_price = await get_entry_price(symbol, client, logger)
        limit_tp_price = round(entry_price * 0.997, pricePrecisions[symbol]) # %1.5
        tp_price = round(entry_price * 0.996, pricePrecisions[symbol]) # %2
